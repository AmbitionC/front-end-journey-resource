# 经典范式：Reflection / Plan-and-Execute / Tool-Use
...
| **描述（Description）** | 告诉 LLM 何时用、何时不用，是工具选择的唯一依据 | "查询实时信息；不适合数学计算" |
...
**粒度原则**：工具粒度不应过细（太多工具让 LLM 难以选择），也不应过粗（一个工具塞太多功能降低精确度）。经验上，单个 Agent 的工具数量以 5-15 个为宜。
...
生产环境强烈推荐 Function Calling。原因：(1) JSON schema 约束大幅减少格式解析错误；(2) 支持并行工具调用（多个 tool call 同时返回）；(3) 不消耗额外 token 写 `Thought:`/`Action:` 前缀；(4) 模型专门针对该格式做了 fine-tune，准确率更高。ReAct 文本格式主要用于不支持 Function Calling 的模型，或需要显式可见 Thought 的场景。
