...
[G-Eval 原始论文](https://arxiv.org/abs/2303.16634)给出了一种具体实现：先让 Judge 根据任务与评估维度生成细化评分步骤，再用 form-filling 方式输出评分；论文在摘要和对话生成任务上比较了该方法与人工判断。这个结果说明它适合被当作需要校准的评分器设计，而不是对所有 Agent 任务都成立的通用正确性证明。
...
- [MT-Bench 与 Chatbot Arena 的原始研究](https://arxiv.org/abs/2306.05685)分析了 position、verbosity 和 self-enhancement bias：答案顺序、冗长程度与候选模型来源都可能影响 Judge，三者不应混为一种“自家模型偏好”。
...
- 对于 LLM Judge 评估，先在独立人工标注集上分别测量各 Judge 的误差，再决定是否聚合；多个 Judge 取平均可以降低部分随机波动，但不能自动抵消共享的系统偏差。（参见 [MT-Bench 与 Chatbot Arena 论文](https://arxiv.org/abs/2306.05685)）
