...
Text-to-SQL 不能只靠向量相似度猜字段。应维护一份可版本化的术语目录，把“动销”“GMV”“大盘”“近 7 天”等业务表达映射到规范实体、指标和时间口径；同义词只负责召回候选，最终仍由语义层校验指标、维度与允许的 join。

当领域关系复杂时，可以用轻量本体描述实体类型、上下位关系和可连接路径，例如“门店属于区域”“退款是订单事件”。本体帮助模型缩小 schema 与 join 候选，但不能替代数据库约束和权限校验。遇到一词多义时应返回候选解释并澄清，而不是静默选择一个 SQL 口径。
