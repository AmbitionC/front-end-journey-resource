...
线上任务量很大时，不应把每条轨迹都再交给一个 LLM。先对全量请求计算确定性信号，例如终态、工具错误、重复调用、步骤数、Token、重试次数、Skill 命中和路径不变量；再按风险、异常和版本分层采样，用 LLM Judge 检查开放式质量；最后只把高风险、Judge 分歧和新型失败交给人工校准。这样形成“规则全量筛查 → 风险采样 → Judge → 人工”的漏斗，既能发现过程偏离，也能控制评测成本。

过程指标不能替代结果指标。工具调用更少未必更好，路径更长也未必错误；它们用于定位异常和选择复核样本，最终仍要结合任务成功、环境终态、安全约束与用户价值判断。
...
候选生成集、调优集和最终验收集要隔离；否则 Agent 可能只记住已经看过的失败。快照至少绑定代码/Prompt、工具 Schema、模型、依赖镜像、随机种子、评测集和 Judge 版本。失败时回滚的不只是配置，还包括索引、缓存和已产生的外部副作用。

Harness 负责构造任务、限制资源、采集轨迹和执行判分；被测 Agent 不得修改自己的验收规则。对开放任务可用模型裁判辅助，但高风险失败、裁判分歧和发布边界仍需人工校准。
