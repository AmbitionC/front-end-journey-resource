![六瓣技能图：Foundations、Build、Evaluate、Operate、Secure、Collaborate；中心为“可可靠交付 Agent 系统”，外圈每域分 beginner/practitioner/advanced，并标出可验证证据而非工具品牌](https://font-end-journey-resources.oss-cn-hangzhou.aliyuncs.com/images/agent-engineer-skill-map-domains-v1.webp)
*图：沿图中的节点与箭头阅读，重点是foundational、build、evaluate、operate、secure、collaborate 六个能力域建立依赖与熟练度证据。*
...
这是整个体系的地基。你不需要从头推导 Transformer 的数学，但要理解注意力、Token 和上下文窗口等影响系统行为的概念。推理参数（temperature、top-p、max_tokens）的语义与可用范围依模型 API 而异；模型选型应在目标任务上联合比较质量、延迟和成本，而不是记住某个品牌层级。
...
这是技能树的核心层之一。ReAct（Reasoning + Acting）是有代表性的 Agent 执行范式，理解推理、行动和观察如何形成反馈循环，有助于比较其他控制流。

除了执行范式，Agent 还需要三项核心能力：**记忆**（短期会话记忆 vs 长期持久记忆）、**规划**（任务拆解与子任务编排）、**工具调用**（Tool Use / Function Calling 的机制与边界）。这三项能力的组合方式，决定了你的 Agent 能解决多复杂的问题。（参见 [GitHub Certified: Agentic AI Developer (beta)](https://learn.microsoft.com/en-us/credentials/certifications/agentic-ai-developer/)）

这部分的深度内容在知识库「**AI 智能体**」章节，是其中的重点，建议多花时间。
...
Tracing（链路追踪）让你能看到每一次 LLM 调用的输入输出；评估框架让你量化 Agent 的准确率和稳定性；调试技巧让你在 Agent 表现异常时快速定位问题。（参见 [OpenAI Agents SDK](https://openai.github.io/openai-agents-python/)）
...
[SFIA 9 责任级别](https://sfia-online.org/en/sfia-9/responsibilities) 用自主性、复杂度、影响和责任描述能力成熟度；技能图谱应以可观察产出分级，而不是按掌握了多少工具品牌打勾。
