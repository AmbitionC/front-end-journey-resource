ANN 与 HNSW 向量检索算法需要把“机制是什么”“边界在哪里”“怎样验证”放在同一条学习路径中。本文以 [Efficient and robust approximate nearest neighbor search using Hierarchical Navigable Small World graphs](https://arxiv.org/abs/1603.09320) 对“HNSW 分层图结构、构建与查询算法的原始论文”的说明为事实边界，并用 [pgvector HNSW documentation](https://github.com/pgvector/pgvector) 校准“HNSW 在 PostgreSQL 中的参数、查询、过滤和迭代扫描行为”。文中的代码和工程方案用于解释这些机制；涉及具体版本、默认值或部署行为时，应再回到所链接的一手资料确认。

![ANN 与 HNSW 向量检索算法的核心机制与验证路径](https://font-end-journey-resources.oss-cn-hangzhou.aliyuncs.com/images/hnsw-layered-graph-search-v1.webp)
*图：ANN 与 HNSW 向量检索算法的核心组件、信息流与验证边界。*
...
图方法将向量集合构建为近似近邻图（Approximate Nearest Neighbor Graph），查询时从入口节点出发沿边"贪心爬坡"，每步移动到距查询向量更近的邻居直到收敛。图方法在同等召回率下查询速度最快，但需额外存储图结构。**HNSW** 是工业界最广泛使用的图索引算法，Milvus、Qdrant、Weaviate、pgvector 等向量数据库均以它为核心。
