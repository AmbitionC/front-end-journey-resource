...
当你在开发自己的 Python 包（如一个内部 RAG 工具库）时，`pip install -e .` 会在 `site-packages` 中创建一个指向源码目录的软链接，修改代码后无需重装即可生效。这对 AI 工程中频繁迭代的工具链尤为实用。
...
**Q：虚拟环境的隔离原理是什么？**
激活虚拟环境时，脚本将 `.venv/bin` 插入 `PATH` 最前面，并设置 `VIRTUAL_ENV` 环境变量。Python 解释器启动时会检测 `VIRTUAL_ENV`，优先搜索该目录下的 `site-packages`，从而实现与系统 Python 完全隔离。
