![从 tidy DataFrame 到 Seaborn semantic mapping（x/y/hue/style）再到 Matplotlib Figure/Axes/Artist；展示同一数据分别用分布、关系、分类图回答不同问题，配色通过色盲安全检查](https://font-end-journey-resources.oss-cn-hangzhou.aliyuncs.com/images/matplotlib-seaborn-layered-grammar-v1.webp)
*图：沿图中的节点与箭头阅读，重点是区分 Matplotlib Figure/Axes 底层控制与 Seaborn semantic mapping，并强调图形选择、尺度和可访问性。*

---

Matplotlib 是 Python 数据可视化的底层引擎，几乎涵盖所有图表需求；Seaborn 建立在 Matplotlib 之上，专为统计图表设计，用更少的代码产出更美观的结果。对于 AI/Agent 工程师而言，这两个库是训练监控、模型评估和数据管道调试不可缺少的工具。（参见 [Seaborn tutorial](https://seaborn.pydata.org/tutorial.html)）
...
[Matplotlib Quick Start](https://matplotlib.org/stable/users/explain/quick_start.html) 将 Figure 作为完整画布、Axes 作为绘图区，并由 Artist 表示可见元素；复杂图表优先使用显式的面向对象接口。
...
# 阻塞式 pyplot 流程通常先保存再 show，避免窗口关闭后当前 Figure 被注销
...
在常见阻塞式 pyplot 流程中，`show()` 返回时窗口可能已关闭且 Figure 从 pyplot 注销，所以通常先 `savefig` 再 `show`。但这不是“show 永远清空 Figure”：[`plt.show()` 文档](https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.show.html)区分阻塞/非阻塞模式；若保留 `fig` 引用，仍可调用 `fig.savefig()`。代码应明确使用哪一个 Figure，而不是依赖隐式当前图。
...
- [Matplotlib pyplot.show documentation](https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.show.html)
