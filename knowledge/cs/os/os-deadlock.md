...
两个事务以相反顺序更新同两行,就构成了循环等待。和应用层不同的是:**数据库内置了死锁检测器**(如 MySQL InnoDB 的 wait-for graph 检测),会自动发现死锁环,然后**回滚代价较小的那个事务**(报 `Deadlock found; try restarting transaction`),让另一个继续。换句话说,数据库默认采用的是「检测 + 恢复」策略,并把恢复的责任部分交给应用——所以业务代码需要**捕获死锁异常并重试事务**。
