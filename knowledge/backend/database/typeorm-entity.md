TypeORM 是 TypeScript 生态中最成熟的 ORM 框架，通过装饰器（Decorator）将类映射到关系型数据库表，同时提供强类型的查询 API。对于构建 AI Agent 后端服务而言，TypeORM 的 Entity 设计尤其重要——会话历史（Session History）的持久化是多轮对话 Agent 实现上下文记忆的基础。
...
`@JoinTable` **只在拥有方（owning side）声明**，另一侧只写 `@ManyToMany`。中间表由 TypeORM 自动管理。
