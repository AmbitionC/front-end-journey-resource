...
[Redis client-side caching 文档](https://redis.io/docs/latest/develop/use/client-side-caching/) 通过 tracking 向客户端发送失效通知，但一致性仍有传播窗口；缓存命中不能证明数据与主存储实时一致。
...
**误区二：缓存不设 TTL，依赖手动删除**  
手动删除逻辑有遗漏风险，Redis 内存会持续增长直到触发 OOM 或 `maxmemory-policy` 强制淘汰。**所有缓存 key 必须设置 TTL**，逻辑过期方案也应在 Redis 层设置一个兜底 TTL。（参见 [Redis key eviction](https://redis.io/docs/latest/develop/reference/eviction/)）
