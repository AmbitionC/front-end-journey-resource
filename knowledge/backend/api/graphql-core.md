![GraphQL 查询经过 parse/validate 后按 selection tree 调用 resolver；左侧 schema 类型与字段，右侧响应树，展示 nullable field 错误向上冒泡但 data 可部分返回](https://font-end-journey-resources.oss-cn-hangzhou.aliyuncs.com/images/graphql-schema-resolver-execution-v1.webp)
*图：沿图中的节点与箭头阅读，重点是type system、operation、selection set、resolver 执行与错误/nullable 传播按规范分层。*
...
[GraphQL Schema 官方教程](https://graphql.org/learn/schema/) 说明 object、field、argument、query 与 mutation 如何组成客户端可查询的类型契约。
...
[GraphQL September 2025 Specification](https://spec.graphql.org/September2025/) 将解析、验证、执行、响应和 nullable 错误传播分开定义；resolver 的返回值必须放在这套执行与完成值规则中理解。
