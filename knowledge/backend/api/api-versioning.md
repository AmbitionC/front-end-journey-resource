![API 版本生命周期：v1 stable 与 v2 并行，客户端通过明确版本选择路由；v1 依次进入 deprecation notice、Sunset deadline、迁移窗口、关闭，兼容适配层连接两版](https://font-end-journey-resources.oss-cn-hangzhou.aliyuncs.com/images/api-versioning-deprecation-lifecycle-v1.webp)
*图：沿图中的节点与箭头阅读，重点是明确 URL/header/media type 版本选择与 SemVer 不能直接等同 HTTP API 兼容。*
...
| 策略 | 示例 | 优点 | 缺点 | 适用场景 |
|---|---|---|---|---|
| **URL Path** | `GET /api/v2/users` | 直观、易调试、缓存友好、文档好分组 | URL 不够"纯粹"，路由层略冗余 | 对外 API 或跨团队协作，且团队接受显式 URL 版本 |
...
[Semantic Versioning 2.0.0](https://semver.org/) 以明确的 public API 为前提定义 major/minor/patch 兼容规则；HTTP API 的兼容性仍需先约定字段、行为和弃用策略，不能只套版本号。
...
无限期维护旧版本会积累大量技术债务：测试矩阵翻倍、基础库升级受阻、Bug 需要多处修复。应制定明确的废弃时间表，配合 Sunset header 和迁移文档，驱动消费方主动迁移。（参见 [RFC 8594: The Sunset HTTP Header Field](https://www.rfc-editor.org/rfc/rfc8594.html)）
