![资源状态与表示图：客户端用 URI 定位资源，通过 GET/PUT/POST/DELETE 交换 representations；标出 safe、idempotent、cacheable 语义以及统一接口约束](https://font-end-journey-resources.oss-cn-hangzhou.aliyuncs.com/images/rest-resource-http-semantics-v1.webp)
*图：客户端先用 URI 定位资源，再通过不同 HTTP 方法交换表示；方法旁的 safe、idempotent、cacheable 标签是需要分别判断的语义。*
...
[Fielding 的原始论文](https://ics.uci.edu/~fielding/pubs/dissertation/top.htm) 以客户端—服务器、无状态、缓存、统一接口、分层系统和可选按需代码等约束定义 REST；只使用 HTTP 和 JSON 并不能自动满足这些约束。
...
[RFC 9110](https://www.rfc-editor.org/rfc/rfc9110.html) 分别定义方法的安全性、幂等性与可缓存性；这些是 HTTP 语义，不应仅凭接口名称或实现结果自行推断。
