![提交→构建→测试→依赖/密钥扫描→生成 SBOM→签名制品→staging→审批→production；同一不可变 artifact 在环境间晋级，失败回滚到上一签名版本](https://font-end-journey-resources.oss-cn-hangzhou.aliyuncs.com/images/cicd-secure-artifact-promotion-v1.webp)
*图：沿图中的节点与箭头阅读，重点是build/test/scan/sign/deploy、制品不可变、环境审批和回滚做成可审计路径。*
...
[NIST SSDF（SP 800-218）](https://csrc.nist.gov/pubs/sp/800/218/final) 将安全准备、软件保护、受保护生产和漏洞响应纳入开发生命周期，扫描只是其中一个控制点。
...
[SLSA v1.2](https://slsa.dev/spec/v1.2/) 用 provenance 和构建保证描述制品来自何处、由什么构建过程产生；安全发布应晋级同一份可验证制品，而不是在每个环境重新构建。
