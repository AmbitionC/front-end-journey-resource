![Docker 从 Dockerfile 构建多层 image，运行时添加 writable container layer；下方 container runtime 使用 namespace/cgroup 隔离，共享 host kernel；volume 与 network 明确在容器层外](https://font-end-journey-resources.oss-cn-hangzhou.aliyuncs.com/images/docker-image-container-runtime-layers-v1.webp)
*图：沿图中的节点与箭头阅读，重点是准确区分 image layers、container writable layer、namespace/cgroup、volume 与网络。*

---

Docker 将应用文件和依赖打包成镜像，并用容器运行时创建隔离进程，从而减少环境漂移。它不能消除宿主内核、CPU 架构、外部服务和配置差异，因此稳定交付仍需要锁定镜像、声明配置并在目标环境验证。

## 容器 vs 虚拟机：本质区别

[Docker 容器概念文档](https://docs.docker.com/get-started/docker-concepts/the-basics/what-is-a-container/) 将容器描述为隔离进程及其运行所需文件；多个容器共享宿主机内核，因此隔离模型不同于完整虚拟机。
...
[OCI Runtime Specification](https://github.com/opencontainers/runtime-spec/blob/main/spec.md) 规定容器配置、文件系统与生命周期的运行时接口；Docker 镜像构建和 OCI runtime 执行是相邻但不同的责任层。
