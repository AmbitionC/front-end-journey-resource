...
PKCE（Proof Key for Code Exchange，RFC 7636）解决的问题：SPA 和移动 App 无法安全存储 `client_secret`，攻击者若拦截到授权码，可直接用它换 token。PKCE 通过引入 `code_verifier`/`code_challenge` 配对，使授权码即使被截获也无法使用。
