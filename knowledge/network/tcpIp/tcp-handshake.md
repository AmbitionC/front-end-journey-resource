...
TCP 只能根据收到的报文和本地定时器更新状态。若对端突然掉电，来不及发送 FIN 或 RST，本端没有立即可见的链路事件，连接通常仍保持 `ESTABLISHED`。

之后怎样发现取决于是否有流量：本端发送数据会启动重传，连续失败并超过实现规定的重传边界后才向应用报告错误；启用 TCP Keepalive 时，空闲连接可由探测失败发现；应用层心跳和读写超时则能按业务时限更早终止。若长期没有数据、Keepalive 或应用超时，半开连接可能保留很久。对端重启后旧连接状态已经丢失，收到旧四元组的报文时可能返回 RST。

排障时要区分主机掉电、进程退出、网络分区和优雅关闭。只有优雅关闭必然产生 FIN；进程异常退出可能由内核发送 FIN 或 RST；网络分区则往往表现为静默丢包和重传。
