...
description: Use when consuming a mixed fe-journey collection batch from `_inbox/` that contains `meta.json.feJourney`, `candidateKinds`, or `clusterId` and must be aggregated and routed into interview/knowledge updates or private operation/project/skipped reports without duplicate public content.
---
...
4. 按 `candidateKinds` 分流；同一 cluster 可同时贡献面经与知识，但公开面经只能有一篇、知识热度只能计一次：
   - `interview`：按现有 [`curate-interview-posts`](../curate-interview-posts/SKILL.md) 的脱敏、站内去重、题解和树更新规范 upsert。
   - `knowledge`：先语义检索既有内容；命中则补新证据/新角度，未命中才调用 [`generate-knowledge-docs`](../generate-knowledge-docs/SKILL.md)。
   - `operation`：写 `_inbox/_reports/operation-topics.md`。
